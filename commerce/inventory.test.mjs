...
test('production checkout reserves stock and consumes it after verified capture',async t=>{
 ...
 globalThis.fetch=async(url,options={})=>{
  ...
  if(url.endsWith('/LIVEPAY123/capture')){paypalOrder.status='COMPLETED';paypalOrder.purchase_units[0].payments={captures:[{id:'CAPTURELIVE',status:'COMPLETED',amount:{currency_code:'USD',value:'159.36'}}]};return Response.json(paypalOrder);}
 ...
 const outOfState={lines:[{sku:'brc-02',quantity:1}],address:{fullName:'Test Person',addressLine1:'1 Test Street',city:'Albany',state:'NY',postalCode:'12207'}};const outOfStateQuote=await call('/quote',outOfState);assert.equal(outOfStateQuote.status,200);assert.deepEqual({...await outOfStateQuote.json(),items:undefined,address:undefined},{subtotal:14999,discount:0,shipping:0,tax:0,total:14999,currency:'USD',code:'',environment:'live',items:undefined,address:undefined});
 const input={lines:[{sku:'brc-02',quantity:1}],address:{fullName:'Test Person',addressLine1:'1 Test Street',city:'Boston',state:'MA',postalCode:'02108'}};
 assert.equal((await call('/quote',input)).status,200);
 const created=await call('/orders',{...input,expectedTotal:15936});assert.equal(created.status,201);const {paypalId}=await created.json();assert.equal(paypalId,'LIVEPAY123');assert.equal(createdPayload.purchase_units[0].amount.breakdown.shipping.value,'0.00');assert.equal(createdPayload.purchase_units[0].amount.breakdown.tax_total.value,'9.37');
