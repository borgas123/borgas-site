...
test('order API: server totals, account isolation, idempotent capture, verified webhook recovery',async t=>{
 ...
 const DB={prepare(sql){return {bind(...values){const stmt=sqlite.prepare(sql);return {first:async()=>stmt.get(...values),all:async()=>({results:stmt.all(...values)}),run:async()=>stmt.run(...values)};}};}};
