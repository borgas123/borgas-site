...
test('validates address and normalizes state',()=>{assert.equal(normalizeAddress({fullName:'Test Person',addressLine1:'1 Main St',city:'Albany',state:'ny',postalCode:'12207'}).state,'NY');assert.throws(()=>normalizeAddress({}));});
...
test('unconfigured service advertises checkout disabled',async()=>{const response=await worker.fetch(new Request('https://api.example/config'),{});assert.deepEqual(await response.json(),{checkoutEnabled:false,clientId:null});});
